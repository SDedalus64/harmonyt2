...
export default function LookupScreen() {
  ...
  // Search for HTS suggestions when user types
  useEffect(() => {
    if (htsCode.length >= 3 && htsCode.length < 8 && !isTariffLoading && !error) {
      // Search the actual tariff data
      const { suggestions, hasMore, totalFound } = searchTariffData(htsCode);
      setHtsSuggestions(suggestions);

      if (suggestions.length === 0) {
        setHtsSearchMessage('No matching records.');
        setShowHtsSuggestions(true); // Show the dropdown with the message
      } else if (hasMore) {
        setHtsSearchMessage(`Showing ${suggestions.length} of ${totalFound}+ matches. Type more digits to refine.`);
        setShowHtsSuggestions(true);
      } else {
        setHtsSearchMessage('');
        setShowHtsSuggestions(true);
      }
    } else if (htsCode.length < 3) {
      setShowHtsSuggestions(false);
      setHtsSearchMessage('');
    }
  }, [htsCode, isTariffLoading, error]);

  // Search tariff data for matching HTS codes
  const searchTariffData = (searchTerm: string): { suggestions: Array<{code: string; description: string}>; hasMore: boolean; totalFound: number } => {
    const matches: Array<{code: string; description: string}> = [];
    const maxResults = 15; // Increased limit
    let totalFound = 0;

    // Since we can't iterate through all entries, we'll search by trying different code combinations
    // This is a workaround since the tariff service only provides findTariffEntry method

    // Try exact match first
    const exactEntry = findTariffEntry(searchTerm);
    if (exactEntry && exactEntry.hts8.startsWith(searchTerm)) {
      matches.push({
        code: exactEntry.hts8 || searchTerm,
        description: exactEntry.brief_description || ''
      });
      totalFound++;
    }

    // Try padding with zeros for exact length match
    const paddedCode = searchTerm.padEnd(8, '0');
    const paddedEntry = findTariffEntry(paddedCode);
    if (paddedEntry && paddedEntry.hts8.startsWith(searchTerm) && paddedEntry.hts8 !== exactEntry?.hts8) {
      matches.push({
        code: paddedEntry.hts8 || paddedCode,
        description: paddedEntry.brief_description || ''
      });
      totalFound++;
    }

    // Try common variations by incrementing last digits - only if they start with searchTerm
    for (let i = 0; i < 100 && matches.length < maxResults; i++) {
      const variantCode = searchTerm + i.toString().padStart(8 - searchTerm.length, '0');
      if (variantCode.length <= 8) {
        const variantEntry = findTariffEntry(variantCode);
        if (variantEntry && variantEntry.hts8.startsWith(searchTerm) && !matches.some(m => m.code === variantEntry.hts8)) {
          matches.push({
            code: variantEntry.hts8 || variantCode,
            description: variantEntry.brief_description || ''
          });
          totalFound++;
        }
      }
    ...
    // Try with different suffixes for more variations - only if they start with searchTerm
    const suffixes = ['10', '20', '30', '40', '50', '60', '70', '80', '90', '00', '15', '25', '35', '45', '55', '65', '75', '85', '95'];
    for (const suffix of suffixes) {
      if (matches.length >= maxResults) break;
      const variantCode = (searchTerm + suffix).slice(0, 8);
      if (variantCode.length >= searchTerm.length && variantCode.startsWith(searchTerm)) {
        const variantEntry = findTariffEntry(variantCode);
        if (variantEntry && variantEntry.hts8.startsWith(searchTerm) && !matches.some(m => m.code === variantEntry.hts8)) {
          matches.push({
            code: variantEntry.hts8 || variantCode,
    ...
    const hasMore = totalFound >= maxResults;

    return {
      suggestions: matches.slice(0, maxResults),
      hasMore,
      totalFound
    };
  };
  ...
  const handleHtsSelection = (code: string) => {
    setHtsCode(code);
    setShowHtsSuggestions(false);
    // Auto-focus country field after selection
    setTimeout(() => {
  ...
  // Scroll to show unit entry field when results are displayed
  useEffect(() => {
    if (!showInput && result && resultScrollViewRef.current) {
      setTimeout(() => {
        // Scroll to show the unit entry field completely
        // This ensures users see they can interact with it
        const scrollPosition = isTablet() ? 480 : 350; // Adjusted to show the full unit entry field
        resultScrollViewRef.current?.scrollTo({
          y: scrollPosition,
          animated: true
        });
  ...
  const handleSaveAndSearch = async () => {
    ...
    try {
      ...
      // Calculate unit costs if unit count is provided
      let unitCalculations = undefined;
      if (unitCount && parseFloat(unitCount) > 0) {
        const units = parseFloat(unitCount);
        const totalWithRT = result.totalAmount;
        let totalWithoutRT = totalWithRT;
        let rtAmount = 0;

        if (result.components && selectedCountry && hasReciprocalTariff(selectedCountry.code)) {
          const rtComponent = result.components.find(c => c.type === RECIPROCAL_TARIFF_TYPE);
          if (rtComponent) {
            rtAmount = rtComponent.amount;
            totalWithoutRT = totalWithRT - rtAmount;
          }
        }

        unitCalculations = {
          costPerUnitWithRT: totalWithRT / units,
          costPerUnitWithoutRT: rtAmount > 0 ? totalWithoutRT / units : undefined,
          additionalPerUnit: rtAmount > 0 ? rtAmount / units : undefined,
          hasRT: rtAmount > 0
        };
      }

      // Save to history with complete data including unit calculations
      await saveToHistory({
      ...
      // Store the current data before resetting
      const previousHtsCode = htsCode;
      const previousCountry = selectedCountry;
      const previousDeclaredValue = declaredValue;
      const previousFormattedDeclaredValue = formattedDeclaredValue;

      // Reset the form but keep the previous data
      setResult(null);
      setShowInput(true);
      setIsSaved(false);
      setUnitCount('');
      ...
      // Preload the previous data
      setHtsCode(previousHtsCode);
      setSelectedCountry(previousCountry);
      setDeclaredValue(previousDeclaredValue);
      setFormattedDeclaredValue(previousFormattedDeclaredValue);

      // Focus on HTS code input to allow user to modify
      setTimeout(() => {
  ...
  const handleHistoryItemPress = (item: HistoryItem) => {
    ...
    if (item.unitCount) {
      ...
      setFormattedUnitCount(formatNumberWithCommas(item.unitCount));
    } else {
      setUnitCount('');
      setFormattedUnitCount('');
    }
    ...
    if (item.totalAmount !== undefined && item.description !== undefined) {
      setResult({
        ...
        unitCalculations: item.unitCalculations,
      });
      setIsSaved(true);
    }
  };
  ...
  const handleHtsCodeSubmit = () => {
    if (htsCode.trim()) {
      // Automatically open the country dropdown and focus the search field
      countryInputRef.current?.focus();
    }
  };
            ...
            {showInput ? (
              <View style={styles.inputContainer}>
                <View style={styles.inputWrapper}>
                  <TextInput
                    ref={htsCodeInputRef}
                    style={[styles.input, !isTablet() && styles.inputPhone]}
                    placeholder="Enter HTS Code (up to 8 digits)"
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      Alert.alert(
                        'Why 8 Digits?',
                        'HTS codes use 8 digits for tariff classification:\n\n' +
                        '• First 6 digits: International HS code\n' +
                        '• Digits 7-8: U.S. subheading for duty rates\n' +
                        '• Digits 9-10: Statistical suffix (not needed for lookups)\n\n' +
                        'The 8-digit code determines the actual duty rate, while digits 9-10 are only for statistical tracking.',
                ...
                {showHtsSuggestions && (
                  <View style={styles.suggestionsContainer}>
                    ...
                    >
                      {htsSuggestions.length === 0 && htsSearchMessage ? (
                        <View style={styles.noResultsContainer}>
                          <Text style={styles.noResultsText}>{htsSearchMessage}</Text>
                          <Text style={styles.noResultsHint}>
                            Try deleting the last digit or entering a different code.
                          </Text>
                        </View>
                      ) : (
                        <>
                          {htsSuggestions.map((suggestion, index) => (
                            <TouchableOpacity
                              ...
                              onPress={() => handleHtsSelection(suggestion.code)}
                            >
                          ...
                          {htsSearchMessage && (
                            <View style={styles.searchMessageContainer}>
                              <Text style={styles.searchMessageText}>{htsSearchMessage}</Text>
                            </View>
                          )}
                        </>
                      )}
                ...
                {/* Add Reciprocal Tariff Toggle - Only show if country has RT */}
              ...
              <View style={styles.resultHeader}>
                ...
                <View style={styles.resultHeaderButtons}>
                  <TouchableOpacity
                    ...
                    onPress={isSaved ? handleNewLookup : handleSaveAndSearch}
                  >
                    <Ionicons name={isSaved ? "search" : "refresh"} size={18} color={COLORS.white} />
                    <Text style={styles.actionButtonText}>{isSaved ? "Search" : "Save & Search"}</Text>
                  </TouchableOpacity>
                  {!isSaved && (
                    <TouchableOpacity
          ...
          {!showInput && result && (
            <>
              ...
              >
                ...
                {/* Unit Cost Calculation Section */}
                <View style={styles.unitCostSection}>
                  {/* Unit Entry Container */}
                  <View ref={unitEntryRef} style={styles.unitEntryContainer}>
                    <View style={styles.unitInputRow}>
                      <Text style={styles.unitInputLabel}>Est. Unit Count:</Text>
                      <TextInput
                        style={styles.unitInput}
                        value={formattedUnitCount}
                        onChangeText={(text) => {
                          handleUnitCountChange(text);
                          // Scroll down slightly when user starts typing to show all calculations
                          if (text && !unitCount) {
                            setTimeout(() => {
                              // Scroll down by a fixed amount to reveal the unit calculations
                  ...
                  {unitCount && parseFloat(unitCount) > 0 && (
                    <View style={styles.unitCalculationsContainer}>
                      {(() => {
                        const units = parseFloat(unitCount);
                        const totalWithRT = result.totalAmount;

                        // Calculate total without reciprocal tariff
                        let totalWithoutRT = totalWithRT;
                        let rtAmount = 0;

                        // Find and subtract reciprocal tariff component if it exists
                        if (result.components && selectedCountry && hasReciprocalTariff(selectedCountry.code)) {
                          const rtComponent = result.components.find(c => {
                            return c.type === RECIPROCAL_TARIFF_TYPE;
                          });
                          if (rtComponent) {
                            rtAmount = rtComponent.amount;
                            totalWithoutRT = totalWithRT - rtAmount;
                        ...
                        const costPerUnitWithRT = totalWithRT / units;
                        const costPerUnitWithoutRT = rtAmount > 0 ? totalWithoutRT / units : undefined;
                        const additionalPerUnit = rtAmount > 0 ? rtAmount / units : undefined;

                        return (
                          <>
                            <View style={styles.costPerUnitRow}>
                              <Text style={styles.costPerUnitLabel} numberOfLines={1}>
                                {selectedCountry && hasReciprocalTariff(selectedCountry.code) && rtAmount > 0
                                  ? (isTablet() ? 'Cost per Unit (with RT):' : 'Per Unit (w/ RT):')
                                  : (isTablet() ? 'Cost per Unit:' : 'Per Unit:')
                                }
                              </Text>
                              <Text style={styles.costPerUnitValue}>
                                {formatCurrency(costPerUnitWithRT)}
                              </Text>
                            </View>
                            {selectedCountry && hasReciprocalTariff(selectedCountry.code) && rtAmount > 0 && (
                              <>
                                <View style={styles.costPerUnitRow}>
                                  <Text style={styles.costPerUnitLabel} numberOfLines={1}>
                                    {isTablet() ? 'Cost per Unit (without RT):' : 'Per Unit (w/o RT):'}
                                  </Text>
                                  <Text style={styles.costPerUnitValue}>
                                    {formatCurrency(costPerUnitWithoutRT || 0)}
                                  </Text>
                                </View>
                                <View style={styles.costPerUnitRow}>
                                  <Text style={[styles.costPerUnitLabel, styles.rtImpactLabel]} numberOfLines={1}>
                                    {isTablet() ? 'Additional per Unit Cost:' : 'RT Impact/Unit:'}
                                  </Text>
                                  <Text style={[styles.costPerUnitValue, styles.rtImpactValue]}>
                                    {formatCurrency(additionalPerUnit || 0)}
                                  </Text>
                                </View>
                              </>
                            )}
...
const styles = StyleSheet.create({
  ...
  headerContent: {
    ...
    paddingTop: 12,
    marginTop: 72, // Reduced by 50% from 144
  },
  headerLogo: {
    width: 440,
    height: 80,
  ...
  unitEntryContainer: {
    // Container specifically for the unit entry field
  },
  unitInputRow: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  costPerUnitValue: {
    ...
    fontWeight: '700',
    textAlign: 'right',
    minWidth: isTablet() ? 100 : 80, // Smaller min width on phones
  },
  rtImpactLabel: {
    color: COLORS.error,
  },
  ...
  suggestionsScrollView: {
    maxHeight: 300, // Increased from 200 to show more items
  },
  suggestionItem: {
    padding: 12,
    borderBottomWidth: 1,
