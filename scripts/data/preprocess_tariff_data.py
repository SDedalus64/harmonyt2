...
Usage:
  python preprocess_tariff_data.py <input_csv> <output_json> [hts_revision] [--inject-extra-tariffs]
...
ADDITIVE_DUTIES = {
    ...
    'section_201_solar': {
        'name': 'Section 201 - Solar Cells/Modules',
        'countries': 'global',
        'rate': 14.25,  # Current rate for 2024-2025, decreasing to 14.0% in Feb 2025
        'effective': '2018-02-07',
        'note': 'Safeguard tariff on CSPV cells and modules. Includes a 5GW tariff-rate quota for cells. Bifacial panels are currently exempt, but this is expected to be rescinded.',
        'exclusions': [ # Developing countries exempt from the safeguard
            'BR', 'KH', 'ID', 'ZA', 'UA', 'TH', 'TR', # Brazil, Cambodia, Indonesia, South Africa, Ukraine, Thailand, Turkey
            # Add other developing country codes as needed from GSP list
        ],
        # HTS codes for solar cells and modules
        'hts_prefixes': ['8541.42', '8541.43'], # Updated HTS-4 for solar cells and modules
    }
}
...
def is_solar_product(hts_code: str) -> bool:
    """Check if HTS code is for solar products"""
    # Section 201 on solar applies to HTS 8541.42 and 8541.43 as of recent updates
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess tariff data to a clean JSON format.",
    ...
    print(f"Found {section_201_count} entries with Section 201 duties")
    ...
    # Show some examples of Section 201 entries
    print("\nExample Section 201 entries (Solar):")
    example_count = 0
    for entry in entries:
        if entry.get('additive_duties'):
            for duty in entry['additive_duties']:
                if duty['type'] == 'section_201' and example_count < 5:
                    print(f"  {entry['hts8']}: {duty['label']} - {entry['brief_description'][:50]}...")
                    example_count += 1
                    break
