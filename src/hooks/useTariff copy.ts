...
export function useTariff() {
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to initialize tariff service"),
        );
        setIsLoading(false);
      }
    };
  ...
  const findTariffEntry = useCallback(
    (htsCode: string): TariffEntry | undefined => {
      if (!service) {
  ...
  const findTariffEntryAsync = useCallback(
    async (htsCode: string): Promise<TariffEntry | undefined> => {
      if (!service) {
        throw new Error("Tariff service not initialized");
      }
  ...
  const searchTariffEntries = useCallback(
    (searchTerm: string): TariffEntry[] => {
      if (!service) {
        throw new Error("Tariff service not initialized");
      }
